...
import { loginAsHR } from '../services/authService'
...
export default function LoginHR(){
  ...
  async function handleLogin(){
    ...
    setError('')
    try {
      await loginAsHR({email,password})
    ...
    <div className="min-h-screen flex items-center justify-center">
      <div className="w-full max-w-md card p-8">
        <div className="flex flex-col items-center gap-3 mb-4">
          ...
          <h1 className="text-2xl font-semibold">HR / Admin Login</h1>
          <div className="muted">Enter your details to continue as HR/Admin</div>
        </div>
        <div className="space-y-3">
          ...
          <button className="w-full gold-btn p-3 rounded" onClick={handleLogin}>Continue as HR/Admin</button>
        </div>
      </div>
    </div>
  )
